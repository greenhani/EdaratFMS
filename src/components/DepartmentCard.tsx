    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <motion.button 
              onClick={onShowMore}
              className="font-bold text-gray-800 dark:text-white text-base hover:text-teal-600 dark:hover:text-teal-400 transition-all duration-200"
              whileHover={{ x: 4 }}
            >
              {department.name}
            </motion.button>
      ...
      {hasMore && (
        <div className="pt-3 border-t border-white/10 mt-auto">
          <motion.button
            onClick={onShowMore}
            className="flex items-center space-x-1 text-xs text-teal-600 dark:text-teal-400 hover:text-emerald-600 dark:hover:text-emerald-400 font-semibold transition-all duration-200"
            whileHover={{ x: 4 }}
            whileTap={{ scale: 0.95 }}
          >
